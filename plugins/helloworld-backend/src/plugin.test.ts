...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [helloworldPlugin],
    });

    await request(server).get('/api/helloworld/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/helloworld/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/helloworld/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/helloworld/todos/${createdTodoItem.id}`)
